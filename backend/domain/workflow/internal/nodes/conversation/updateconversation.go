...
func (c *UpdateConversation) Invoke(ctx context.Context, in map[string]any) (map[string]any, error) {
	...
	if !ok {
		return nil, vo.WrapError(errno.DeprecatedErrInvalidParameter, errors.New("new conversationName name is required"))
	}

	newConversationName := ncName.(string)

	if agentID != nil {
		return nil, vo.WrapError(errno.DeprecatedErrConversationNodesNotAvailable, fmt.Errorf("in the agent scenario, update conversation is not available"))
	}

	if appID == nil {
		return nil, vo.WrapError(errno.DeprecatedErrConversationNodesNotAvailable, errors.New("conversation update node, app id is required"))
	}
